...
#set page(
  margin: (inside: 3.5cm, outside: 2cm, bottom: 2.5cm, top: 2cm),
  binding: left,
  numbering: "1",
)
...
#set outline(
  title: box(
    ...
    text(weight: "regular")[Spis treści],
  ),
)

#set bibliography(title: [Literatura])

#show heading: it => block(box(inset: (bottom: 1.25em), it))
#show outline: set text(font: "STIX Two Text", size: 12pt, fill: black, weight: "bold")
...
#let author = [Damian Pietryka]

// np. EF-123456, EN-654321, ...
#let studentID = [EF-177140]

#let title = [Implementacja środowiska wykonawczego\ CPDev runtime w języku Rust]
#let titleEN = [Temat pracy po angielsku]
...
/// promotor
#let supervisor = [db inż. Bartosz Trybus]
/// przykład: dr hab. inż. Józef Nowak, prof. PRz

//// promotor ze stopniami naukowymi po angielsku
#let supervisorEN = [Prof. Imię i nazwisko opiekuna, (academic degree)]

#let abstract = [Treść streszczenia po polsku]
#let abstractEN = [Treść streszczenia po angielsku]

#let keywords = [max. 5 słów kluczowych w 2 wierszach, oddzielanych przecinkami]
#let keywordsEN = [max. five keywords in English]

#let font_size = 16pt

#let rodzajPracyPLTitle(rodzajPracyNo) = {
  if rodzajPracyNo == 1 {
    [PRACA INŻYNIERSKA]
  } else if rodzajPracyNo == 2 {
    [PRACA MAGISTERSKA]
  } else if rodzajPracyNo == 3 {
    [PRACA DOKTORSKA]
  } else if rodzajPracyNo == 4 {
    [PROJEKT INŻYNIERSKI]
  } else {
    [wpisz-rodzaj-pracy]
  }
...
#[
  ...
  #align(center)[
    #image(width: 12.5cm, "auxiliary/logoweii.jpg")
    #v(4.5cm)
    #text(size: 17pt, weight: "bold")[#author]
    #v(0.5cm)
    #text(size: 17pt)[#title]
    #v(4cm)
    #text(size: 17pt, weight: "bold")[#rodzajPracyPLTitle(rodzajPracyNo)]
  ]

  #v(1fr)

  #text(size: 17pt)[Opiekun pracy:]

  #text(size: 17pt)[#supervisor]
  #v(5em)
  #align(center)[
    #text(size: 17pt)[Rzeszów, #datetime.today().display("[year]")]
  ]
  #pagebreak()
]
...
#text(fill: red)[
  ...
  Niniejszy tekst, wykorzystujący  styl _weiiszablon.sty_ zawiera informacje o formatowaniu, wielkości czcionek, wyrównania. . . , ale *uwaga*, sama treść nie jest istotna (np. opis wielkości czcionek),
...
Do 20\% objętości pracy. W zależności od charakteru pracy ten rozdział powinien zawierać:
  ...
  [opis tematyki zagadnienia -- aktualny stan zagadnienia],
  [metody i rozwiązania],
  [dyskusja i krytyczna ocena stanu aktualnego],
  [podsumowanie stanu wiedzy, techniki literaturowe itp.],
)
...
Ponad 50\% objętości pracy -- część autorska:
  ...
  indent: 1.25cm,
  numbering: "a)",
  enum.item([założenia -- dane,]),
  enum.item([ opis zastosowanej metody rozwiązania lub analizy,]),
  enum.item(
    [ opis proponowanego rozwiązania, wyniki analizy teoretycznej, obliczenia, projekt konstrukcyjny, procesowy, technologiczny,],
  ),
  enum.item([wyniki badań analitycznych, symulacyjnych lub eksperymentalnych itp.]),
)
...
=== Zalecenia co do sposobu pisania jednostek i symboli wielkości fizycznych

Poniższy podrozdział opracowano na podstawie @Pawluk2001. W trakcie pisania pracy należy zwracać uwagę na sposób oznaczania jednostek i symboli wielkości fizycznych. Przy zapisywaniu jednostek i symboli wielkości fizycznych można wyróżnić zapis w~postaci kursywy (pismo pochyłe) oraz antykwy\footnote{Potoczna nazwa pisma prostego.} (pismo proste).
...
\item wskaźnik dolny, górny, prawo- i lewostronny, ale tylko gdy odnosi się do konkretnej wielkości fizycznej, czyli np. składowa $x$-owa indukcji magnetycznej $B_x$, a nie $B_#emph("x")$,

\item wskaźniki górne i dolne oznaczające dowolną liczbę np. $R_j$, $I^k$, ale nie $R_{1}$, $I^{2}$.
...
\item symbole konkretnych funkcji np. $tg omega t$, a nie $tg omega t$,

\item operatory operacji matematycznych np. pochodne zwyczajne $frac{d x}{d t}$, a nie $frac{d x}{d t}$,

\item symbole liczb o konkretnej wartości np. przenikalność elektryczna próżni $epsilon_0 = 8,8542 \cdot 10^{-12}$ $F \cdot {m}^{-1}$, a nie $epsilon_0 = 8,8542 \cdot 10^{-12}$ ${F} \cdot {m}^{-1}$,

\item indeksy, jeżeli odnoszą się do: obiektów (fizycznych, geometrycznych), czyli, np. natężenie pola elektrycznego w punkcie A to $E_{A}$, a nie $E_A$, zjawisk lub stanów fizycznych, np. moment obciążenia to $T_{L}$, a nie $T_L$, do nazwisk czy też oznaczeń pierwiastków, np. straty w miedzi to $P_{{C u}}$ a nie $P_{C u}$, do charakteru wielkości symbolizowanej przez literę źródłową, np. wartość maksymalna siły to $F_{max}$, a nie $F_{max}$, oznaczeń jednostek miary np. ${M}Omega$, a nie $M {Omega}$.
...
\begin{itemize}[label={--},labelsep=0.4cm,leftmargin=0.6cm]
\item zapisując wartość liczbową wielkości fizycznej po spacji należy podać jej jednostkę, ale nie nazwę jednostki np. $10~{A}$, ale nie $10$ amper czy też $10$ amperów,
...
\item do oznaczeń jednostek nie wolno dopisywać indeksów, np. moc wyjściowa silnika wynosi $P=100$ ${k W}_{o u t}$. W takim przypadku należy zapisać $P_{o u t}=100$ ${k W}$,

\item jednostek nie należy umieszczać w nawiasach kwadratowych, np. $I=1$ ${[A]}$. Odstępstwem od tej zasady mogą być tabele, nagłówki kolumn, opisy osi na wykresach oraz w sporadycznych sytuacjach we wzorach matematycznych (ale tylko wówczas, gdy zależność matematyczna nie wskazuje w~jakiej jednostce wystąpi wartość liczbowa). Przykłady odstępstw zamieszczono w~podrozdziale \ref{Subsec:Rysunki-i-tabele}.
...
\item wartości szczytowe funkcji zmiennej, amplitudę funkcji sinusoidalnej czasu należy zapisać jako np. $U_{m}$,
...
\item kreska nad literą źródłową oznacza wartość średnią, np. $overline{I}$ co jest równoważne $I_{a v}$.
...
\begin{example}
[. . . ] Na rysunku \ref{Fig:wykres} pokazano przykładową zależność prądów pasmowych $i_{p h}$ bezszczotkowego silnika prądu stałego z magnesami trwałymi w funkcji położenia wirnika $theta$.
\end{example}
...
\begin{example}
[. . . ] oraz indukcyjności wzajemnej. W tabeli \ref{Tab:tabela} przedstawiono podstawowe parametry obwodu nieliniowego, zasilanego napięciem trójfazowym.
\end{example}
...
\begin{example}
[. . . ] wyznacza się, na podstawie wyrażenia (\ref{Eq:rownanie}). W nawiasach podano rozmiary czcionek używanych we wzorach
\end{example}
...
automatycznie za pomocą parametru \verb|language|. W niniejszym dokumencie przedstawiono trzy przykłady listingów -- listing \ref{KodMatlab1} to przykład kodu źródłowego Matlaba, poniżej listing \ref{KodPerl1} dla Perla i dołączony z~zewnętrznego pliku listing języka C (\ref{lst:plchr}).\\
%\komentarz{
...
Numeracja linii jest w stylu domyślnie włączona (to przydatne, bo w treści pracy łatwo odwołać się dzięki temu do konkretnego wiersza w kodzie źródłowym), możesz je wyłączyć podając jako parametr \verb|numbers=none|. Więcej szczegółów możesz odnaleźć w sekcji \verb|\lstset| pliku arkusza stylów.
%}

Można również dołączać listingi z~zewnętrznych plików\footnote{i nadać im taki sam (albo inny) wygląd jak ze środowiska \texttt{lstlisting}.}, \textbf{zwłaszcza jeżeli zawierają polskie znaki diakrytyczne}. Można w~tym celu wykorzystać polecenie \verb|lstinputlisting|, które przekoduje znaki na te z~dostępnej czcionki:
\begin{verbatim}
...
Można zauważyć, że oryginalny plik \texttt{start.c} z~katalogu \texttt{auxiliary} zawiera więcej linii, a~tutaj zamieszczono tylko wybrane (od 2 do 6).
...
=== Uwagi końcowe dla użytkowników \LaTeX
Kompilację dokumentu można przyspieszyć dodając opjconalny parametr \texttt{draft} do polecenia \texttt{\symbol{92}documentclass}.
Wówczas rysunki nie będą zamieszczane, a~w~ich miejscu pojawi się prostokąt z obrysem.
Ponadto tryb ten zaznacza również fragmenty tekstu wychodzące na margines sygnalizowane przez komunikaty podczas kompilacji np. \texttt{Overfull \symbol{92}hbox (14.2264pt too wide) in paragraph at lines ...}.
Takie naruszenia marginesów oznaczane są czarnym prostokątem na końcu wystającej linii.
Gdy linia ewidentnie nie mieści się w~obszarze strony, to również ten prostokąt nie zostanie zaznaczony.
Dlatego warto przejrzeć finalną postać tekstu, aby wykryć potencjalne niewielkie naruszenia marginesu.
...
wykaz został zaprezentowany w niniejszym dokumencie w odpowiednim rozdziale, oznaczonym jako ,,Literatura''  (pozycja nr @str to zasoby internetowe,
@Jakubczyk1997 -- książka, @Barski2011 -- artykuł w czasopiśmie, @dokum -- karta katalogowa).
...
badaw\-czo-rozwojowy) natomiast myślnik -- stosowany w zdaniach wtrąconych -- zapisuje
